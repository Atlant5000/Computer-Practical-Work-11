...
namespace ConsoleApp20
{
    ...
    internal class Program
    {
        ...
        public static void Value()
        {
            ...
            int e = Input("EF=");
            int f = Input("FD=");
            Console.WriteLine($"Периметр первого треугольника={Perimetr(a,b,c)};Площадь первого треугольника={Math.Round(Square(a,b,c),2)}");
            Console.WriteLine($"Периметр второго треугольника={Perimetr(d,e,f)};Площадь второго треугольника={Math.Round(Square(d,e,f), 2)}");
            Console.ReadLine();
        }
    }
}
